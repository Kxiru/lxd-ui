...
const NetworkDevicePanel: FC<Props> = ({
  ...
  const getDefaultEgressIngress = () => {
    // Return NIC device default egress/ingress actions if set.
    if (
      formik.values.security_acls_default_egress_action &&
      formik.values.security_acls_default_ingress_action
    ) {
      return {
        Egress: formik.values.security_acls_default_egress_action,
        Ingress: formik.values.security_acls_default_ingress_action,
      };
    }
