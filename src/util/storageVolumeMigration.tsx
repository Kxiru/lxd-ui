...
import { hasLocation, linkForVolumeDetail } from "./storageVolume";
...
export const useStorageVolumeMigration = ({
  ...
  const handleSuccess = () => {
    ...
    const memberPath = hasLocation(volume)
      ...
      : "";
    const oldMigratePoolUrl = linkForVolumeDetail({
      name: volume.name,
      project: volume.project,
      pool: volume.pool,
      type: volume.type,
    } as LxdStorageVolume);
    const newMigratePoolUrl = linkForVolumeDetail({
      name: volume.name,
    ...
    if (type === "pool") {
      ...
      if (oldMigratePoolUrl !== newMigratePoolUrl) {
        navigate(newMigratePoolUrl);
      }
    }
  ...
  const handleMemberMigrate = () => {
    ...
    const payload: Partial<LxdStorageVolume> = {
      ...
      source: {
        ...
        pool: volume.pool,
        volume_only: false,
        project: volume.project !== volume.project ? volume.project : undefined,
